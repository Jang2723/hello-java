...
public class StreamMain {
    public static void main(String[] args) {
        ...
        // 원시타입 스트립(int, double, long)
        int[] intArray = {1, 23, 14, 53, 22, 15, 10, 6}; //Wrapper class가 아니어서 Stream으로 전달 x
        IntStream intStream = Arrays.stream(intArray);

        // 문자열을 문자 스트림
        String email = "edujeeho@gamil.com";
        IntStream charStream = email.chars(); // e,d,u,j, ... 하나씩 나오는 데이터 흐름
        ...
        */
        ...
        nameListStream
                ...
                // filter : 전달받은 함수를 바탕으로 참인 데이터만 남기는 Stream
                 .filter(name -> name.contains("d")) // 선별 작업, 중간작업
                //          매개변수 -> 조건
...
//                    return nameLower + nameUpper;
                ...
                // .map(name -> name.length()) // 반환형이 바뀐다.(Integer)
                // sorted : 데이터를 받아 전달받은 기준으로 정렬을 한다.
                .sorted(Comparator.reverseOrder())
                // 종결 작업
                // 새로운 데이ㅓ 모음을 바탕으로 결론을 내린다.
                //.forEach(System.out :: println);
                // .forEach(name -> System.out.println(name)) 과 같은 의미
                // forEach : 남은 데이터를 각각 전달받은 함수의 인자로 전달
                // filter, map, sorted 모두 반환형이 Stream이어서 forEach가 동작
                // forEach 는 Stream이면 동작
                // toList : 데이터를 새로운 리스틑로 만들어서 반환
                // .toList();
...
//                .forEach(System.out :: println);
    ...
    // 입력은 무엇이며 결과는 무엇일까? (매개변수) (반환타입) 물어보는 것
