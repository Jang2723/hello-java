...
public class SetMain {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 20; i++) {
            Pigeon pigeon = new Pigeon(i % 10); // 비둘기 나이 설정 0~9,0~9 20마리
            listHoles.add(pigeon);
            setHoles.add(pigeon);
        }

        System.out.println("리스트의 비둘기: " + listHoles.size());    //20
        System.out.println("집합의 비둘기: " + setHoles.size());      //20 -> 10
        ...
        System.out.println(skillSet.removeAll(thirdSkills)); // skillSet에서 thirdSkills를 뺀것
        System.out.println(skillSet);
        // 집합 비우기
        skillSet.clear();
        System.out.println(skillSet);
